...
class SpanHighlightingComponent2(initialSpans: List[List[Span]]) {

  sealed trait SpanHighlightingStatus
  case object NoSpan extends SpanHighlightingStatus
  @Lenses case class Highlighting(index: Int, anchor: Int, endpoint: Int) extends SpanHighlightingStatus
  ...
  @Lenses case class SpanHighlightingState(
                                            spans: Map[Int, List[Span]],
                                            status: SpanHighlightingStatus)
  object SpanHighlightingState {
    def initial = SpanHighlightingState(
      initialSpans.zipWithIndex.map {
        case (spans, idx) => idx -> spans
      }.toMap.withDefaultValue(Nil),
      NoSpan)
  }

  case class SpanHighlightingContext(
                                      setSpan: Map[Int, List[Span]] => Callback,
                                      hover: Int => Int => Callback,
                                      touch: Int => Int => Callback,
                                      cancel: Callback)
  ...
  class SpanHighlightingBackend(scope: BackendScope[SpanHighlightingProps, SpanHighlightingState]) {
    ...
    def setSpan(spans: Map[Int, List[Span]]): Callback =
    ...
    def hover(props: SpanHighlightingProps)(index: Int)(endpoint: Int) = modStateWithUpdate {
    ...
    def touch(props: SpanHighlightingProps)(index: Int)(wordIndex: Int): Callback = modStateWithUpdate {
