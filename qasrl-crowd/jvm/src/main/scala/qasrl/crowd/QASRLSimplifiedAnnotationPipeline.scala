...
class QASRLSimplifiedAnnotationPipeline[SID : Reader : Writer : HasTokens](
  ...
  def info(): Unit = {
    ...
    println(s"Coverage disqualification id: ${genCoverageDisqualType.getQualificationTypeId}")
    println(s"Accuracy disqualification id: ${genAccDisqualType.getQualificationTypeId}")
    println()
